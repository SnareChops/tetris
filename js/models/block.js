var Block = function(shape, x, y) {
  ...
  self.moveRight = function(board) {
    if (self.collision(board, { x: self.position.x + 1, y: self.position.y }, self.shape.coordinates)) {
      return false;
    }
    self.position.x++;
    return true;
  ...
  self.rotateRight = function(board) {
    ...
    if (self.collision(board, self.position, newCoordinates)) {
      return false;
    }
  ...
  self.collision = function(board, position, coordinates) {
    return !coordinates.every(function(coord) {
      ...
      for (var row in board.blob) {
        if (
          !board.blob[row].every(function(cell) {
